...
function getStatusBadge(status: string) {
  switch (status) {
    case "completed": return <span className="badge-success">Complétée</span>;
    case "pending": return <span className="badge-warning">En attente</span>;
    case "cancelled": return <span className="badge-destructive">Annulée</span>;
    default: return null;
  }
}
...
function getPaymentStatusBadge(paymentStatus: string) {
  switch (paymentStatus) {
    case "paid": return <span className="badge-success">Payé</span>;
    case "partial": return <span className="badge-warning">Partiellement payé</span>;
    case "unpaid": return <span className="badge-destructive">Impayé</span>;
    default: return <span className="badge-muted">N/A</span>;
  }
}
...
export default function Sales() {
  ...
  const handleCreateSale = async (data: any) => {
    ...
    const prev = qc.getQueryData<any[]>(['sales']);
    qc.setQueryData<any[]>(['sales'], old => [optimistic, ...(old ?? [])]);
    ...
    try {
      ...
      qc.setQueryData(['sales'], prev); // rollback
      setIsModalOpen(true);
      showErrorToast("Erreur", e.response?.data?.message || "Impossible de créer la vente");
    }
  };
  ...
  return (
    <MainLayout title="Ventes" subtitle="Historique et gestion de vos transactions commerciales">
        ...
        <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
          <div className="flex flex-1 gap-3 w-full md:w-auto">
            <div className="relative flex-1 md:w-80">
              ...
              <Input placeholder="Rechercher une vente..." className="pl-10"
      ...
      {filteredSales.length === 0 ? (
        <EmptyState icon={ShoppingCart} title="Aucune vente"
          description="Commencez par enregistrer votre première vente"
          actionLabel="Nouvelle vente" onAction={() => setIsModalOpen(true)} />
      ) : (
            ...
            <TableBody>
              {filteredSales.map((sale) => (
                <TableRow key={sale._id || sale.id} className="table-row">
                  ...
                  <TableCell><span className="font-medium text-foreground">{sale.clientName}</span></TableCell>
      ...
      <SaleFormModal open={isModalOpen} onOpenChange={setIsModalOpen} onSubmit={handleCreateSale} />
    </MainLayout>
  );
}
